...
-- 3. Invalid or unusual transaction values
...
SELECT
    ...
    SUM(CASE WHEN CreditCardID IS NULL THEN 1 ELSE 0 END) AS orders_without_card,
    SUM(CASE WHEN CreditCardID IS NOT NULL THEN 1 ELSE 0 END) AS orders_with_card
FROM salesorderheader
...
SELECT
    ...
    END) AS customers_without_orders
FROM customer AS c
